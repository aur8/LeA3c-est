...
void Model::Load_model(const std::string& filename)
{
    ...
    bool ret = tinyobj::LoadObj(&m_attrib, &m_shapes, &m_materials, &warn, &err, objFile.c_str());
    ...
    for (const auto& shape : m_shapes)
    {
        ...
        for (size_t i = 0; i < shape.mesh.num_face_vertices.size(); i++)
        {
            ...
            for (size_t v = 0; v < fv; v++)
            {
                ...
                vertex.position =
                    glm::vec3(m_attrib.vertices[3 * index.vertex_index + 0], m_attrib.vertices[3 * index.vertex_index + 1], m_attrib.vertices[3 * index.vertex_index + 2]);

                vertex.normal = glm::vec3(m_attrib.normals[3 * index.normal_index + 0], m_attrib.normals[3 * index.normal_index + 1], m_attrib.normals[3 * index.normal_index + 2]);

                vertex.texCoords =
                    glm::vec2(m_attrib.texcoords[2 * index.texcoord_index + 0], m_attrib.texcoords[2 * index.texcoord_index + 1]);
